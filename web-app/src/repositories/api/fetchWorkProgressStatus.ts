...
import { DEVICE_PROGRESS_STATUS } from "../constants";
import { BaseResPayload } from "./common";

// Type of Request Params object
type ReqParams = {
  deviceId: number; // 管理ID
};

// Type of Response Payload object
type ResPayload = BaseResPayload & {
  status: DEVICE_PROGRESS_STATUS; // ステータス
  review_comment: string;
  status_code: number;
  error_code: number;
};

// API to fetch review status for selected device
export function fetchWorkProgressStatus(params: ReqParams) {
  return client
